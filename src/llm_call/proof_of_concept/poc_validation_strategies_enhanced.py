"""
Enhanced POC Validation Strategies with V4 improvements.

This module contains validation strategies for the POC, including:
- Basic validators (response not empty, JSON, field checks)
- AI-assisted validators using Claude agents with MCP tools
- Support for dynamic tool configuration via MCP

Documentation:
- Validation patterns: See core/validation/
- MCP integration: See poc_claude_proxy_server.py
"""
...
class PoCResponseNotEmptyValidator(AsyncValidationStrategy):
    """Validates that LLM response contains non-empty content."""
    ...
    async def validate(self, response: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        elif hasattr(response, "error"):  # Generic error attribute
            is_error_response = True
            error_message = str(response.error)
...
class PoCJsonStringValidator(AsyncValidationStrategy):
    """Validates that response content is valid JSON."""
...
class PoCFieldPresentValidator(AsyncValidationStrategy):
    """Validates field presence/absence and optionally checks expected values."""
    ...
    async def validate(self, response: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        if self._present:  # Check for presence
            if field_is_present:
                if self._expected_value is not None:
                    ...
                    if actual_value == self._expected_value:
                        ...
                        return ValidationResult(
                            valid=False,
                            error=f"Field '{self._field_name}' present but value '{actual_value}' != expected '{self._expected_value}'."
                        )
                return ValidationResult(valid=True, debug_info={self._field_name: data_to_check.get(self._field_name)})
            else:
                return ValidationResult(
                    valid=False,
                    error=f"Required field '{self._field_name}' is missing.",
                    suggestions=[f"Ensure LLM includes '{self._field_name}'."]
                )
            ...
            if not field_is_present:
                ...
                return ValidationResult(
                    valid=False,
                    error=f"Field '{self._field_name}' is present but should be absent.",
                    suggestions=[f"Ensure LLM excludes '{self._field_name}'."]
                )


class PoCAgentTaskValidator(AsyncValidationStrategy):
    """
    AI-assisted validator that delegates validation to a Claude agent.
    
    The agent can use MCP tools and returns structured JSON validation results.
    """
    
    def __init__(self,
                 task_prompt_to_claude: str,
                 validation_model_alias: str = "max/default_validator_agent",
        ...
        Args:
            task_prompt_to_claude: Prompt template with placeholders like {TEXT_TO_VALIDATE}
            validation_model_alias: Model to use for validation (max/ prefix for Claude proxy)
            mcp_config: MCP tool configuration for the agent
            success_criteria: Dict defining how to interpret agent's response
        """
    ...
    @property
    def name(self) -> str:
        # Generate a name from the first few words of the task prompt
    ...
    async def validate(self, response_to_validate: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        # Extract content from the primary LLM's response
        ...
        # Build the final prompt with substitutions
        final_agent_prompt = self._task_prompt_to_claude.format(
            ...
            ORIGINAL_USER_PROMPT=context.get("original_llm_config", {}).get("messages", [{}])[-1].get("content", "N/A")
        )
        ...
        agent_task_messages = [
            {
                "role": "system",
                "content": "You are a diligent validation agent. Execute the user's validation task precisely using any specified tools. Respond ONLY with a structured JSON object detailing your findings as per the user's requested JSON schema."
            },
            {
                "role": "user",
                "content": final_agent_prompt
            }
        ]
        ...
        try:
            ...
            elif hasattr(agent_response_dict, "choices") and agent_response_dict.choices:
                agent_report_str = agent_response_dict.choices[0].message.content or ""
            
            if not agent_report_str:
                return ValidationResult(valid=False, error=f"[{self.name}] Validation agent returned empty content.")
            
            logger.debug(f"[{self.name}] Validation agent raw report: {agent_report_str}")
            agent_report = json.loads(agent_report_str)  # Agent MUST return valid JSON
            
            # Apply success criteria to interpret agent report
            is_valid = True  # Assume valid unless criteria say otherwise
            reasoning = agent_report.get("reasoning", "Agent provided no explicit reasoning.")
            details = agent_report.get("details", {})

            # Check different success criteria
            if "agent_must_report_true" in self._success_criteria:
            ...
            if "all_true_in_details_keys" in self._success_criteria and isinstance(details, dict):
                required_keys = self._success_criteria["all_true_in_details_keys"]
                if not isinstance(required_keys, list):
                    required_keys = [required_keys]
                for key in required_keys:
                    if not details.get(key, False) == True:  # Must be explicitly True
                        is_valid = False
                        reasoning += f" [Criteria Fail: details key '{key}' not true or missing.]"
                        break
            
            if "must_not_contain_string_in_reasoning" in self._success_criteria and isinstance(reasoning, str):
            ...
            if is_valid:
                ...
                return ValidationResult(
                    valid=False,
                    error=f"Agent validation failed: {reasoning}",
            ...
            return ValidationResult(
                ...
                debug_info={"raw_agent_response": str(agent_response_dict)}
            )
...
if __name__ == "__main__":
    """Test the validation strategies."""
    import asyncio
    
    async def test_validators():
        logger.info("Testing POC validation strategies...")
        
        # Test 1: Response not empty
        validator = PoCResponseNotEmptyValidator()
        ...
        assert result.valid == True, "Should validate non-empty response"
        logger.success("✓ Response not empty validator works")
        
        # Test 2: JSON validator
        json_validator = PoCJsonStringValidator()
        json_response = {
            "choices": [{"message": {"content": '{"key": "value"}'}}]
        }
        result = await json_validator.validate(json_response, {})
        assert result.valid == True, "Should validate valid JSON"
        logger.success("✓ JSON validator works")
        
        # Test 3: Field present validator
        field_validator = PoCFieldPresentValidator("name", present=True)
        field_response = {
            "choices": [{"message": {"content": '{"name": "test", "age": 25}'}}]
        }
        result = await field_validator.validate(field_response, {})
        assert result.valid == True, "Should find present field"
        
        # Test absence
        absence_validator = PoCFieldPresentValidator("password", present=False)
        result = await absence_validator.validate(field_response, {})
        assert result.valid == True, "Should validate absent field"
        logger.success("✓ Field present/absent validator works")
        
        # Test 4: Field with expected value
        value_validator = PoCFieldPresentValidator("status", expected_value="active")
        value_response = {
            "choices": [{"message": {"content": '{"status": "active"}'}}]
        }
        result = await value_validator.validate(value_response, {})
        assert result.valid == True, "Should validate expected value"
        logger.success("✓ Field value validator works")
